...
function add(text,type='bot',keyboard,photo){const el=document.createElement('div');el.className='msg '+type;el.innerHTML=md(text);
  // D-048: schema di impiattamento (data URI PNG) mostrato anche nel simulatore, non solo su
  // Telegram — utile per la prova del progettista in Fase 0 (NEXT.md).
  if(photo){const img=document.createElement('img');img.src=photo;img.alt='Schema di impiattamento';img.className='plating-schema';el.append(img)}
  if(keyboard){const k=document.createElement('div');k.className='keyboard';keyboard.flat().forEach(label=>{const b=document.createElement('button');b.textContent=label;b.onclick=()=>send(label);k.append(b)});el.append(k)}messages.append(el);messages.scrollTop=messages.scrollHeight;return el}
async function send(text,voice=false){
  ...
  try{
    ...
    add(d.reply.text,'bot',d.reply.keyboard,d.reply.photo);
...
async function beginNewChapter(){const r=await fetch('/api/message',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({userId,text:'/start',name:'Tester'})});const d=await r.json();add(d.reply.text,'bot',d.reply.keyboard,d.reply.photo)}
async function boot(){const r=await fetch(`/api/user/${encodeURIComponent(userId)}`);if(r.ok){const u=await r.json();const pending=u.pendingDplus,due=pending?.dueAt?Date.parse(pending.dueAt):Infinity;if(pending&&Date.now()>=due){const x=await fetch('/api/dplus',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({userId})});const y=await x.json();add(y.reply.text,'bot',y.reply.keyboard,y.reply.photo)}}await beginNewChapter()}
boot();
