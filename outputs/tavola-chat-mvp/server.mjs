...
const server=http.createServer(async(req,res)=>{try{const url=new URL(req.url,'http://local');
  ...
  if(req.method==='POST'&&url.pathname==='/telegram/webhook'){const update=await body(req);await telegramUpdate(update);return json(res,200,{ok:true})}
