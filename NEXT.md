...
Ultimo aggiornamento: 28 agosto 2026 (D-038: corretto un secondo bug segnalato dal progettista in test reali — bloccato sulla proposta di seppia senza modo di cambiare percorso; corretti gli stati proposal e mode, 66/66 test, verificato dal vivo con una vera chiamata al laboratorio, deployato sulla VM. D-036 e D-037 confermati deployati sulla VM in una sessione precedente: `git log` su tavola-server allineato a `origin/main`. Restano da verificare dal vivo su Telegram reale: D-034 (consenso, orario D+1, consegna proattiva), D-036 (techniqueMapId), D-037 (fix "Altra idea") e D-038 (cambio di percorso da proposal/mode).)
