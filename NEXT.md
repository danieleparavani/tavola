...
- [x] Sincronizzare sulla VM il fix D-027 ora presente su GitHub — fatto: `git reset --hard origin/main` (ora al commit `d72ca78`, storia pulita); `npm run check` e `npm test` superati sulla VM (52/52); `tavola.service` e `tavola-tunnel.service` riavviati e `active`; verificato funzionalmente via `/api/message` locale (nuovo utente arriva a `collecting_people` con i tasti 1/2/3/4/5+); utente di test rimosso da `data/pilot.json`. Resta da verificare il flusso a tasti aprendo una conversazione vera con `@tavola_cucina_bot` su Telegram.
...
- [x] Valutare se la latenza della generazione completa meriti un messaggio intermedio su Telegram — implementato: il webhook ora risponde subito (200 OK) invece di attendere l'intera elaborazione (evita il timeout già osservato sul livello Gourmet), e quando l'utente sceglie un livello (stato `difficulty_choice`) riceve un messaggio “Sto pensando alla proposta...” prima della chiamata lenta al laboratorio. Applicato prima su server.mjs sulla VM, verificato con 53/53 test (incluso un nuovo test di regressione sul comportamento fire-and-forget del webhook), poi replicato su GitHub (commit `16df30c` per il codice, `c8624f4` per il test) e VM riallineata a `origin/main`. Non ancora verificato con una conversazione reale su Telegram: vedi EVIDENCE.md.
