...
Ultimo aggiornamento: 28 agosto 2026 (D-036 e D-037 confermati deployati sulla VM in una sessione successiva: `git log` su tavola-server mostra HEAD allineato a `origin/main` al commit `3750efe`, 64/64 test superati direttamente sulla VM, `tavola.service`/`tavola-tunnel.service` `active`, `/api/status` con `labConnected:true`. Restano da verificare dal vivo su Telegram reale: D-034 (consenso, orario D+1, consegna proattiva), D-036 (techniqueMapId) e D-037 (fix "Altra idea").)
